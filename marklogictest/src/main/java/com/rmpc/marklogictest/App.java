 ...
 * 1) create a database
 * 
 * 2) setup a forest which maps where the documents are stored in the file
 * system - can have many of those naming convention is usually
 * database_name-01, database_name-02, database_name-0n
 * 
 * 3) setup a port that allows a program to communicate with marklogic database
 * - java client is built on top of the REST API so a rest api instance is
 * required when using the java api
 * 
 * command: curl --anyauth --user admin:admin -X POST -d@"news-config.json" -i
 * -H "Content-type: application/json" http://localhost:8002/v1/rest-api
 * 
 * json config file: { "rest-api": { "name": "8011-news-rest", "database":
 * "news", "port": "8011" } }
 *
 * 
 */
public class App {
	 ...
	 */
	static {
		client = DatabaseClientFactory.newClient(HOST, PORT, DATABASE, new DigestAuthContext(USERNAME, PASSWORD));
	...
	/**
	 * Having marklogic generate the uri for us. We just need to let marklogic know
	 * its a json document
	 * 
	 * @param content
	 */
	private static void createDocumentAndURI(String content) {
		jsonDocMgr.create(jsonDocMgr.newDocumentUriTemplate("json"), new StringHandle(content));
	}

	/**
	 * to write to a particular collection we need to specify it with a meta data
	 * handle as bellow
	 * 
	 * @param uri
	 * @param content
	...
	private static void createTextDocument(String uri, String content) {
		...
		DocumentMetadataHandle defaultMetadata = new DocumentMetadataHandle().withCollections("April 2014");

		// Give the handle some content
		handle.set(content);
		// Write the document to the database with URI from docId and content from
		// handle
		textDocMgr.write(uri, defaultMetadata, handle);
	}

	/**
	 * write multiple docs synchronously - if any doc fails to write the whole batch
	 * fails as a transaction
	 * 
	 * @param docsMap
	 */
	 ...
	 * 
	 * @param uri
	 *            - document uri that we want to update
	 * @param patchContext
	 *            - property that will be updated, needs to be an object
	 * @param patchContent
	 *            - content being inserted in the patched property
	 */
	private static void patchDocument(String uri, String patchContext, String patchContent) {
		...
		DocumentPatchHandle patch = patchBldr.insertFragment(patchContext, Position.LAST_CHILD, patchContent).build();

		jsonDocMgr.patch(uri, patch);
	}

	/**
	 * to delete a document we just call the delete() method with the uri of the
	 * document
	 * 
	 * @param uri
	 */
	...
	private static void searchDocument(String searchCriteria) {
		...
		// query.setCollections("drinks"); // if we want to search in a particular
		// collection of the database
		// SearchHandle results = queryMgr.search(query, searchHandle);
		// JacksonDatabindHandle<Drink> results = queryMgr.search(query, new
		// JacksonDatabindHandle<>(Drink.class));
	...
	private static void structedSearchDocument(String searchCriteria) {
		...
		StructuredQueryDefinition query = qb.and(qb.word(qb.jsonProperty("name"), "manhanthan"));
		SearchHandle results = queryMgr.search(query, handle);
		parseResults(results);

		SearchHandle handle1 = new SearchHandle();

		results = queryMgr.search(new StructuredQueryBuilder().term("true", "Mocha"), handle1);
		parseResults(results);
	}
	...
	public static void main(String[] args) {
		...
		// createDocumentAndURI("{name: \"Chocolate Mocha\", size: \"Grandé\", tasty:
		// true}");
